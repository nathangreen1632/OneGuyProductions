        ...
        return (
          ...
          >
            ...
            {expandedCard && (
              <div
                id={panelId}
                className="flex-1 overflow-y-auto p-4 sm:p-6 pr-1 -mr-1 custom-scrollbar"
              >
                {order.updates.map((update: OrderUpdateEntry): React.ReactElement => {
                  const uniqueKey: string =
                    `${update.user}-${update.timestamp}-${update.message.slice(0, 20)}`;
                  return (
                    <div key={uniqueKey} className="relative pl-4 py-3 border-l-2 border-[var(--theme-border-red)]/30">
