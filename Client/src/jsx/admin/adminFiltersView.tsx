...
export default function AdminFiltersView({
                                           ui,
                                         }: Readonly<AdminFiltersViewProps>): React.ReactElement {
  return (
    <div className="rounded-2xl bg-[var(--theme-surface)] p-3 shadow-sm grid gap-2 md:grid-cols-4">
      ...
      <select
        ...
        onChange={(e: ChangeEvent<HTMLSelectElement>): void =>
          ui.set('status', e.target.value as any)
        }
        className="rounded-xl border border-[var(--theme-border-blue)] bg-transparent p-2 text-sm outline-none"
      >
      ...
      <select
        value={ui.assigned}
        onChange={(e: ChangeEvent<HTMLSelectElement>): void =>
          ui.set('assigned', e.target.value as any)
        }
        className="rounded-xl border border-[var(--theme-border-blue)] bg-transparent p-2 text-sm outline-none"
      >
      ...
      <select
        ...
        onChange={(e: ChangeEvent<HTMLSelectElement>): void =>
          ui.set('updatedWithin', e.target.value as any)
        }
        className="rounded-xl border border-[var(--theme-border-blue)] bg-transparent p-2 text-sm outline-none"
      >
