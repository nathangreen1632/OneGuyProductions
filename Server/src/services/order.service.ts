import {OrderModel, OrderCreationAttributes, OrderInstance} from '../models/order.model.js';
...
export async function handleNewOrder(data: OrderCreationAttributes): Promise<OrderHandlingResult> {
  ...
  try {
    const created: OrderInstance = await OrderModel.create(data);
    dbSuccess = true;
    orderId = created.id;
  } catch (err: unknown) {
    const dbErrorMessage: string = err instanceof Error ? err.message : 'Unknown database error';
  ...
  try {
    ...
    const emailErrorMessage: string = err instanceof Error ? err.message : 'Unknown email error';
  ...
  return { dbSuccess, emailSuccess, orderId };
}
